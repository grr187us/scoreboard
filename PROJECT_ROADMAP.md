...
| Deep-dive audit | Fifteen ranked findings from the September 5, 2026 whole-project review — five critical fixes, five missing features, five improvements — are recorded under **Deep-dive audit** below with file-level evidence. The five lightest (C1, C2, C3, I3, and the isolation slice of I1) were implemented and verified the same day; C4, C5, F1–F5, I2, I4, I5 and the rest of I1 are open, three of them blocked on owner decisions (A-1, B-1, overtime rules). |
...
Result: **712 tests, 15 failures, 3 errors** (September 5, 2026, after the deep-dive fixes). Re-run on September 5, 2026 after
...
Until the deep-dive fixes a sixteenth failure appeared whenever `SCOREBOARD_DATA_DIR` was **not** set: `tests/integration/test_persistence.py::DataLocationTests::test_the_default_root_is_an_absolute_per_user_location` called `resolve_paths()` with no override, and the resolver's documented order is override → environment variable → *the operator's remembered folder* → platform default, so the test was asserting against this machine's live `data-location.json` (`…\Desktop\Scoreboard Logs`). That was a test-isolation defect, not a resolver bug; the test now removes the remembered-folder layer for the call, and the count is 15 with or without the override.

**How the 15 split.** Eleven are mechanical: they assert the pre-follow-up values (`720.0` where the unified pregame clock now gives `1800.0`, `11:57` where it gives `29:57`, acceptance where a quarter move now returns `CONFIRMATION_REQUIRED`) and can be rewritten to the documented behavior in an afternoon. Four are **blocked on question A-1** — the two `ClockCouplingTests`, `ExpirationHistoryTests.test_game_clock_expiry_clears_a_running_play_clock_durably`, and the play-clock half of `test_start_matches_the_engine_and_materializes_into_state` — because rewriting them means deciding whether a game-clock Start always blanks the play clock, which is a football-rules question for the owner and the officials, not a code decision. Keep the two buckets separate so "green" is never conflated with "A-1 answered".
...
| F3 | No crowd-facing game-state messages: no `FLAG`, `TIMEOUT`, `INJURY`, or `DELAY` indicator, and no timeout countdown clock. When play stops the board freezes with no explanation; while a penalty is being worked in the Field Assistant the wall still shows the old down and distance. The v2 editor's free text is decorative and pre-positioned, not a live toggle (§10.8). | `views/spectator/*`, `docs/UX_AND_LAYOUT.md` §10.8 | M | Open. The knowledge base lists "Crowd prompts" under Graphics. |
| F4 | No team presets, colours, or logos. Both names are retyped from a blank state every week (F-010); `GameState` carries no team colour, logo, or abbreviation; the v2 editor has *layout* presets but no *team* presets. | `domain/state.py`; `presentation/layout.py` presets | S–M (presets) / L (visual identity) | Open. Knowledge base: "Visual system" and "Rosters". |
...
| # | Finding | Evidence | Weight | Status |
|---|---|---|---|---|
| I1 | The suite is not a release gate: 15 legacy failures, one test that read the operator's live config, three browser checks that cannot run here, and no CI, lint, or type-check anywhere (no `.github/`, no ruff/mypy/pyright config). | "Automated suite failure inventory" above | 11 mechanical rewrites S; A-1 bucket blocked; CI S | ✅ **Partially implemented (the lightest slice).** The machine-dependent failure is fixed: `DataLocationTests` now isolates itself from the operator's remembered folder and the count is 15 with or without `SCOREBOARD_DATA_DIR` (it was 16 without). The 15 are now explicitly split in the inventory into **11 mechanical** and **4 blocked on A-1**. Still open: rewriting the 11, answering A-1, installing Node/Playwright, and a `.github/workflows` run with the relative-link check. |
| I2 | The dev environment is not reproducible: `.venv/pyvenv.cfg` points at **Blender 4.5's bundled Python**, documented nowhere, and the pins were validated against that build; Node/Playwright resolve only from a Codex runtime cache. | `.venv/pyvenv.cfg`; `tests/README.md` browser section | S (document) / M (rebuild) | Open. Rebuild from python.org 3.11, install Node/Playwright locally, document both in `tests/README.md`. |
...
| I4 | One global one-level Undo covers scoring, quarter changes, and Field Assistant composite plays alike (F-014); the `LAST:` strip shows only that entry. A second action of any kind — including an accidental shortcut — forecloses fixing the first, with no view of what is being given up. The durable history already records everything. | `application/service.py` `undo_entry`; `views/operator/index.html:100-101` | M–L | Open. |
...
3. **The physical 1366×768 check.** The layout has twice been measured in Chromium at the equivalent CSS viewports, most recently after the Task 10 health-strip change, but never on a 1366×768 Windows display at 100% and 125% scaling under WebView2. The 44×44 accessibility target for `Reopen Display` is unmet at 32 px and needs the operator layout re-flowed.
...
9. **Browser tooling on this host.** Node.js and Playwright are not installed, so the two `tests/ui/` checks cannot run here and the Task 10 operator JavaScript was exercised against a stub bridge in a browser rather than by those tests. Install the tooling, or run the suite where it exists, before release.
