...
| Automated suite | Bundle B verification: 405 tests passed in 46.010 seconds with an isolated data-folder override, including the real-browser spectator and keyboard suites. An unisolated run has one environment-specific failure because this laptop's saved data-folder preference is named `Scoreboard Logs`; it is not a product failure and the isolation override is the documented test/rehearsal path. |
