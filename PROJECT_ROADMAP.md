...
| Automated suite | **Green.** A full discovery run on the rebuilt `.venv` with Node.js on PATH, September 6, 2026 (`.\.venv\Scripts\python.exe -m unittest discover -s tests -v`, with `SCOREBOARD_DATA_DIR` pointed at an isolated folder) reported **750 tests, 0 failures, 0 errors, 3 skipped** in 51 seconds. `compileall`, `pip check`, and `tools/check_markdown_links.py` all pass. The 3 skips are the tests blocked on question A-1 — see "Automated suite failure inventory" below, now a historical record rather than a list of expected failures. A skip is not an A-1 answer. Earlier discovery runs on September 5, 2026 (712/15/3, then 615/16/3 after the Field Assistant volunteer screen, then 703/16/3 after the layout editor v2 rebuild) and the September 6 run before the day's I1/I2 fixes (750/15/3) are kept for lineage in that section; none of those was a regression. |
| Repository status | Work is on `feature/field-status`, ahead of `main` (`2b29942`) by the Field Assistant, layout editor v1/v2/v3, and documentation commits. The v2 editor commit `7986fc6` and the deep-dive fixes commit that follows it are not yet pushed; `origin/feature/field-status` is at `2af419d`. The Phase 1 foundation commit `6f9fc18` was pushed and independently cloned cleanly. `phase-2-audit` and `feature/layout-editor` are earlier branches retained for history. The Field Assistant and the presentation layout editor exist **only as uncommitted working-tree changes** until the September 5 documentation-reconciliation commit lands. The `AGENTS.md` change and the `docs/agents/` directory are now in-scope, tracked modifications rather than the unrelated bystander files an earlier revision of this row described. The September 6, 2026 deep-dive I1/I2 work — the rebuilt `.venv` (python.org CPython 3.11.9, replacing the undocumented Blender-bundled Python), the root `package.json`/`package-lock.json`, `.github/workflows/ci.yml`, and `tools/check_markdown_links.py` — is likewise uncommitted working-tree state as of this writing. |
...
| Deep-dive audit | Fifteen ranked findings from the September 5, 2026 whole-project review — five critical fixes, five missing features, five improvements — are recorded under **Deep-dive audit** below with file-level evidence. C1, C2, C3, and I3 were implemented and verified September 5, 2026 (the isolation slice of I1 was also done that day); I1 and I2 were completed September 6, 2026 (see the Improvements table and the Decision Log above). C4, C5, F1–F5, I4, and I5 remain open, three of the fifteen findings overall blocked on owner decisions (A-1, B-1, overtime rules). |
...
Result as of September 6, 2026, on the rebuilt `.venv` with Node.js on PATH:
**750 tests, 0 failures, 0 errors, 3 skipped** in 51 seconds, with
`SCOREBOARD_DATA_DIR` isolated. `compileall`, `pip check`, and
`tools/check_markdown_links.py` also pass. The table below is now a
