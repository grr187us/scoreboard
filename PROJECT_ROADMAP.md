...
| 2 | Core scoreboard MVP | Produce a dependable local Windows scoreboard with correct football controls and fullscreen output. | 🟡 In progress; Tasks 1-9 and 11 implemented and locally verified, audited September 5, 2026. Task 10, Task 12, and all hardware/release evidence remain. |
...
| Item | Current state |
|---|---|
| Active phase | Phase 2. Tasks 1-9 and 11 are implemented; Task 10 and Task 12 remain. Phase 0's hardware gate is open in parallel |
...
| Implementation status | Tasks 1-9 implemented and verified locally: separate recovery startup, authoritative lifecycle and play-clock visibility, immediate command publication, responsive spectator game/countdown layout, keyboard safety and generated shortcut help. The September 5 audit added expiration recording, application-version-safe recovery, and a whole-game rehearsal. Task 11 then produced a verified offline one-folder package at version 0.1.0. Task 10 (display selection) and Task 12 (rehearsal) remain unstarted. |
| Automated suite | 287 Python tests pass on this host; the two `tests/ui/` browser checks require Node.js and Playwright and error explicitly without them. |
...
1. **Answer question A-1** from the audit — whether a game-clock Start should always blank the play clock, given that the game clock also starts on the ready-for-play. This is a football-rules question for the owner and the officials, not a code decision, and it affects the stadium's only play-clock display. It is the one open item that can visibly mislead the field.
2. **Task 10 — display selection, fullscreen, and failure recovery.** The only remaining implementation task, and the one the stadium test feeds directly. It needs a two-display machine, which this development host does not have.
3. **Rebuild the package after Task 10.** The current build carries Task 1-era display behaviour, because Task 11 was built out of order. No release may use the pre-Task-10 package.
4. **Work the `docs/PACKAGING.md` checklist on the target laptop** — clean machine, network disabled, SmartScreen, startup time.
