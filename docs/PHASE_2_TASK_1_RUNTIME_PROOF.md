...
.\.venv\Scripts\scoreboard-proof.exe
```
...
```powershell
.\.venv\Scripts\scoreboard-proof.exe --display-index 99
```
...
```powershell
.\.venv\Scripts\scoreboard-proof.exe --display-index 0 --auto-close-after-seconds 30
```
...
1. Run `scoreboard-proof`. Confirm the operator placeholder and a borderless fullscreen spectator placeholder open; confirm the spectator is on the selected second display.
...
6. Close the operator. In Task Manager, confirm the `scoreboard-proof` Python process exits and no child process remains.
