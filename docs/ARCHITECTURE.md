...
`presentation/layout.py`'s `LAYOUT_SCHEMA_VERSION` is now **2**. `validate_layout` accepts a document whose `schema_version` is 1 or 2 and always normalizes it to 2; a v1 document is upgraded in place, with every new property filled from its default and exactly one warning (`SCHEMA_UPGRADED`) rather than an error, so a layout saved before this change keeps opening. A document carries an optional top-level `background` (`{"color": "#RRGGBB"}`, defaulted with a warning if absent) and an optional `elements` list — up to `MAX_ELEMENTS = 24` free-standing `text`, `image`, or `box` items, each with its own geometry, stacking order, and (for `text`) the same style properties a widget now carries (font family from a fixed ten-entry system-font table, letter spacing, text transform, a shadow/outline effect, background fill, border, corner radius, padding). Elements never participate in widget-overlap validation, and only a `text` element is held to the safe-area boundary — `image` and `box` need only stay inside the canvas, which is how a full-bleed backdrop is built. An `image` element's `src` is a `data:image/{png,jpeg,gif,webp};base64,...` URI: the decoded bytes are validated by magic-byte sniffing, capped at `MAX_IMAGE_BYTES = 2_000_000` per image, and capped at `MAX_TOTAL_IMAGE_BYTES = 6_000_000` decoded across every image in one layout — so an image is stored inline in `layouts.json` itself, never as a separate file or a network reference, and a layout with too many or too-large images is rejected outright rather than silently truncated. The schema module remains pure (no I/O) and its public functions still never raise on malformed input; `clamp_layout` repairs element geometry the same way it already repairs widget geometry.
...
A v3 document adds one top-level key, `screens`, alongside the unchanged v1/v2 top-level shape. The **top level of the document remains the game screen** — its own `safe_area`, `background`, `widgets` (the fifteen game widgets), and `elements`, exactly as v2 defined them, so every existing test, file, and code path that reads the top level keeps working unmodified. `screens.pregame` and `screens.halftime` are each a complete mini-document with the same four keys — their own `safe_area`, `background`, `widgets`, and `elements` — but drawn from a *different* widget registry: eight **event widgets** (home/away team name and score, phase label, countdown title, countdown, and a warmup line) rather than the fifteen game widgets, because the pregame and halftime screens show different information than the game board. `screens` missing from a v3 document (but not from a v1/v2 upgrade, where `SCHEMA_UPGRADED` already covers it) is filled from defaults with one warning (`MISSING_SCREENS`); a single missing screen fills from its own default with `MISSING_SCREEN`; an unrecognized key under `screens` warns (`UNKNOWN_SCREEN`); anything not shaped as expected is a hard error (`SCREENS` / `SCREEN`).
