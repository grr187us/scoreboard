...
- **September 14, 2026 — live sync: the helper adopts every push (PL-2).** The first official game showed the stale gate in practice: every control-panel change (a down, a score, a timeout, a quarter) bumped the revision, the helper answered the very next 10 Hz push with **FIELD STATUS CHANGED ELSEWHERE — RE-SYNC REQUIRED** and a disabled Confirm, and the operator "had to refresh the field assistant all the time". The helper was already receiving the live state; it was rejecting it. Now every `applyView` push and every preview or finalize response adopts `model.revision` as the base revision and re-renders; there is no stale banner, and Confirm is never disabled because of a change made elsewhere. A pending press is re-previewed against the new state (without disabling Confirm) so its label stays truthful; Python recalculates the raw draft at finalize time regardless. The draft ball follows the board on every push until the operator moves it (click, drag, arrow key, nudge button, or the typed yard line); from then on it is theirs until a committed action or the explicit **Discard draft & reload** button (the old **Reload from scoreboard**) gives it back. `finalize_field_action(committed, baseRevision)` is unchanged, so Python still refuses a genuine race — a control-panel command landing between the helper's last push and its Confirm — with `STALE_REVISION`; the helper then shows a five-second toast, re-seeds the ball from the board, re-previews, and one more Confirm finishes the play. Section 4.5 and FA-19/FA-29 below are updated; the September 5 draft-ownership amendment still holds for a touched ball.
...
- On open, the assistant reads one complete current snapshot. Every later push, preview response, and finalize response is adopted as the new base revision (live sync, September 14, 2026); nothing on this screen ever needs a manual reload.
- The draft ball follows the scoreboard until the operator moves it. Once touched (click, drag, arrow key, nudge button, or the typed yard line) it is the operator's until a committed action or **Discard draft & reload** gives it back. The ten-per-second refresh push updates every read-only readout and may re-draw a touched draft on the mirrored side at a quarter boundary, but never moves it, and never silently changes what Confirm would send.
- If the current operator screen, keyboard shortcut, or another accepted action changes the revision while a press is pending, the helper re-previews that press against the new state; Confirm stays enabled. Only a genuine race — a change landing between the last push and the Confirm press — is refused by Python (`STALE_REVISION`); the helper shows a short toast, re-seeds the ball from the board, re-previews, and one more Confirm completes the play. No stale draft is ever merged.
- **Discard draft & reload** is the explicit "throw away what I pressed" path. It clears the pending press and gives the ball back to the board.
...
| FA-19 | Stale draft | Manual field edit or score changes revision while draft is open; separately, a change lands between the last push and Confirm | An ordinary change is adopted from the push and the pending press re-previews with Confirm still enabled; a genuine race is refused once with `STALE_REVISION`, re-seeds, and completes on the next Confirm; no stale write occurs. |
...
| FA-29 | Draft ownership | Move the ball by nudge, arrow key, drag, and yard selection while refresh pushes continue; then Discard draft & reload, then confirm a play; separately, leave the ball untouched and move it from the control panel | A touched spot stays exactly where the operator put it across any number of pushes; only a committed action, a refused race, and Discard draft & reload re-seed it; an untouched ball follows the board within one push; a quarter boundary re-draws the same spot on the mirrored side. |
