...
A presentation layout editor for spectator-board placement, sizing, color, free text, images, and simple shapes is implemented as v2 (September 5, 2026; `docs/UX_AND_LAYOUT.md` §10, `docs/PHASE_2_BACKLOG.md`). It remains presentation-only: it advances no state revision, submits no command, and does not add media, OBS, animations, sponsor rotation, networking, per-resolution layouts, physical controllers, SVG images, downloaded fonts, or a way to bind free text to an authoritative field; the pregame/halftime event countdown board is not editable in v2. See `docs/PHASE_2_BACKLOG.md` for the full v2 exclusion list.
