...
| Play-clock behavior when game clock starts | When the game clock transitions from stopped to running, clear the play clock: stop it and make it visibly blank. If the game clock was already running, a play-clock countdown continues normally to zero unless an operator clears or changes it. | Owner decision | Define how an expected zero during a live play is presented without a misleading alert. **Open (audit A-1):** this assumes the game clock starts at the snap. Under NFHS-style rules the game clock also starts on the ready-for-play, with the play clock already running; the rule as written would blank the stadium's only play-clock display in those situations. Confirm with the owner and officials before live use. |
| Play-clock behavior when game clock stops | A real game-clock STOP transition, including natural expiration at 0:00, clears a running play clock and makes it blank. A redundant STOP while the game clock is already stopped leaves an independently running play clock unchanged. | Owner decision, September 5, 2026 | Revisit only if officials require a different stopped-clock workflow. |
...
| Quarter change | Stop both clocks, then require confirmation if either was running. When the target is `1st`, `2nd`, `3rd`, `4th`, or `OT` and the resulting game clock is `0:00`, load its stopped 12:00 default; never overwrite a nonzero game clock. | Owner decision | Overtime uses the same 12:00 default until local rules are confirmed. |
...
| F-029 | A transition that lands on `1st`, `2nd`, `3rd`, `4th`, or `OT` with a resulting game clock of `0:00` MUST load the stopped configured quarter length. It MUST preserve any nonzero value and MUST NOT load a game clock for `PRE`, `HALF`, or `FINAL`. A transition that loads the clock is not undoable. | Forward, backward, direct-selection, halftime, nonzero, and Undo command tests. |
...
| F-052 | When the game clock changes from running to stopped through an explicit Stop command or natural expiration, the application MUST stop and clear a running play clock, making the spectator play-clock area blank. A Stop command issued while the game clock is already stopped MUST NOT affect an independently running play clock. Natural expiration MUST leave the state revision unchanged while recording both the game-clock expiry and the system-caused play-clock clear durably. | Fake-time command and bridge-tick tests, including repeated ticks after expiry. |
