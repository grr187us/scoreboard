...
(function (global) {
  ...
  /* The "soccer" registry: the soccer in-game board (spec section 5.1).
   * Mirrors scoreboard.presentation.soccer_layout.SOCCER_WIDGET_IDS. Pre-game
   * and halftime stay on the "event" registry above, unchanged -- soccer
   * only ever needs a game-board registry of its own. */
  var SOCCER_WIDGET_IDS = [
    "home_name", "home_score", "away_name", "away_score",
    "game_clock_label", "game_clock_value",
    "period",
    "status_message", "status_clock",
  ...
  /* Mirrors scoreboard.presentation.soccer_layout.SOCCER_WIDGET_FIELDS. */
  var SOCCER_WIDGET_FIELDS = {
    "home_name": "teams.home.name",
    "home_score": "teams.home.score",
    "away_name": "teams.away.name",
    "away_score": "teams.away.score",
    ...
    "status_message": "status.display",
    "status_clock": "status.clock_display",
    "home_shots": "soccer.home.shots_display",
    "away_shots": "soccer.away.shots_display",
    "home_saves": "soccer.home.saves_display",
    "away_saves": "soccer.away.saves_display",
    "home_corners": "soccer.home.corners_display",
    "away_corners": "soccer.away.corners_display",
    "home_fouls": "soccer.home.fouls_display",
    "away_fouls": "soccer.away.fouls_display",
    "home_yellow": "soccer.home.cards.yellow_display",
    "away_yellow": "soccer.away.cards.yellow_display",
    "home_red": "soccer.home.cards.red_display",
    "away_red": "soccer.away.cards.red_display",
    "shootout_home": "soccer.shootout.home_display",
    "shootout_away": "soccer.shootout.away_display"
  };

  /* period's Standard/Short format choice. Mirrors
   * scoreboard.presentation.soccer_layout.SOCCER_WIDGET_FORMAT_FIELDS. */
  var SOCCER_WIDGET_FORMAT_FIELDS = {
  "period": {
    "short": "period_display_short"
  }
};

  /* Mirrors scoreboard.presentation.soccer_layout.SOCCER_WIDGET_TEXTS. */
  var SOCCER_WIDGET_TEXTS = {
    "game_clock_label": "GAME CLOCK"
  };

  /* Mirrors scoreboard.presentation.soccer_layout.SOCCER_OPTIONAL_WIDGET_IDS. */
  var SOCCER_OPTIONAL_WIDGET_IDS = [
    "status_message", "status_clock",
    "home_yellow", "away_yellow", "home_red", "away_red",
    "shootout_home", "shootout_away"
  ];
  ...
  /* Mirrors scoreboard.presentation.soccer_layout.default_soccer_layout()
   * (minus its `screens` key -- soccer's pre-game/halftime screens are
   * football's own event screens, unchanged, so they use DEFAULT_SCREENS
   * above, not a soccer-specific fallback). This is the "soccer" registry's
   * fallback document (spec section 5.2, the Soccer Grid preset). */
  var SOCCER_DEFAULT_LAYOUT = {
  "schema_version": 3,
  "name": "Default",
  "safe_area": {
    "top": 0.04,
  ...
  "widgets": {
    "home_name": {
      ...
      "cut_corners": "all",
      "border_style": "solid",
      "font_family": "bahnschrift_condensed",
      "letter_spacing": 0.0,
      "text_transform": "uppercase",
      "text_effect": "none",
      "padding": 0.0,
  ...
  var SOCCER_OPTIONAL_LOOKUP = buildOptionalLookup(SOCCER_OPTIONAL_WIDGET_IDS);
  ...
  var REGISTRIES = {
    ...
    soccer: {
      ids: SOCCER_WIDGET_IDS,
      fields: SOCCER_WIDGET_FIELDS,
      texts: SOCCER_WIDGET_TEXTS,
      optionalLookup: SOCCER_OPTIONAL_LOOKUP,
      fallback: SOCCER_DEFAULT_LAYOUT
    }
  };

  /* Format tables keyed the same way as REGISTRIES, for the `boardKind`
   * gate in applyModel() below (spec section 2.3). The "event" kind has no
   * format choices, so it is simply absent here, exactly as before. */
  var FORMAT_FIELDS_BY_KIND = {
    game: WIDGET_FORMAT_FIELDS,
    soccer: SOCCER_WIDGET_FORMAT_FIELDS
  };

  function registryForKind(kind) {
    if (kind === 'soccer') {
      return REGISTRIES.soccer;
    }
   ...
   * spectator page's `#canvas`). `kind` is `"game"` (default), `"event"`,
   * or `"soccer"` (spec section 2.3). */
  function build(container, kind) {
    if (!container) {
      return;
    }
    var resolvedKind = kind === 'event' ? 'event' : (kind === 'soccer' ? 'soccer' : 'game');
    // Soccer mode: the layout editor's bootstrap builds its board root as
    // 'game' (views/layout/layout.js is frozen). A page whose board root
    // carries a static data-game-kind="soccer" attribute asks for the soccer
    // registry in that one case; football's pages carry no such attribute.
    if (resolvedKind === 'game' && container.getAttribute('data-game-kind') === 'soccer') {
      resolvedKind = 'soccer';
    }
  ...
  function applyModel(container, model) {
    ...
    var isGameBoard = boardRoot.dataset.boardKind === 'game' || boardRoot.dataset.boardKind === 'soccer';
    var hiddenByModel = isGameBoard ? read(model, 'board.hidden_widgets') : null;
    if (!Array.isArray(hiddenByModel)) hiddenByModel = [];
    var hiddenPrefixes = isGameBoard ? read(model, 'board.hidden_element_prefixes') : null;
    ...
    for (var index = 0; index < registry.ids.length; index += 1) {
      ...
      try {
        ...
        var formatsTable = FORMAT_FIELDS_BY_KIND[boardRoot.dataset.boardKind];
        var formats = formatsTable && formatsTable[id];
  ...
  global.ScoreboardBoard = {
    ...
    SOCCER_WIDGET_IDS: SOCCER_WIDGET_IDS,
    SOCCER_WIDGET_FIELDS: SOCCER_WIDGET_FIELDS,
    SOCCER_WIDGET_TEXTS: SOCCER_WIDGET_TEXTS,
    SOCCER_OPTIONAL_WIDGET_IDS: SOCCER_OPTIONAL_WIDGET_IDS,
    SOCCER_WIDGET_FORMAT_FIELDS: SOCCER_WIDGET_FORMAT_FIELDS,
    SOCCER_DEFAULT_LAYOUT: SOCCER_DEFAULT_LAYOUT,
